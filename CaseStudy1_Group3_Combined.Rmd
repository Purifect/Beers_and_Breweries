...
author: "Group 3 Data Junkies - Chris Roche, Puri Rudick, and Ryan Kinney"
output:
...
```{r}
breweryDataOrig = as.data.frame(breweryDataOrig)
Median.in.each.state = medianData
names(Median.in.each.state)[1] <- "state"
Median.in.each.state$state <-
  str_replace_all(string=Median.in.each.state$state, 
                  pattern=" ", repl="")
centroid_labels <- 
  utils::read.csv(system.file("extdata",
                              paste0("us_", "states", "_centroids.csv"), 
...
labels <- 
  merge(x = centroid_labels, 
        y = Median.in.each.state, 
        by.x = "abbr", 
        by.y = "state", 
        all.x=TRUE)
names(labels)[6] <- "breweries_count"
plot_usmap(data = Median.in.each.state, values = "IBU", labels=FALSE) + 
  scale_fill_continuous(low = "white", 
                        high = "darkgreen", 
                        name = "Mean IBU", 
                        label = scales::comma) + 
...
labels <- 
  merge(x = centroid_labels, 
        ...
        all.x=TRUE)
names(labels)[6] <- "breweries_count"
plot_usmap(data = Median.in.each.state, values = "ABV", labels=FALSE) + 
  scale_fill_continuous(low = "white", 
                        high = "darkblue", 
                        name = "Mean ABV", 
                        label = scales::comma) + 
